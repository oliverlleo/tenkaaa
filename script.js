(() => {
  ...
  if (canvas && !reduce && !mobileMq.matches) {
  ...
  if (finePointer && !mobileMq.matches && !reduce) {
  ...
  if(preview && finePointer && !mobileMq.matches && !reduce){
  ...
  if (isMobile) {
    intro.from('.brand',{y:-12,opacity:0,duration:.55})
      .from('.hero-title .line>span',{yPercent:72,duration:.82,stagger:.09},'-=.25')
      .from('.hero-kicker,.hero-side',{y:18,opacity:0,duration:.6,stagger:.08},'-=.55')
      .from('.orbit',{scale:.75,opacity:0,duration:.65,ease:'power3.out'},'-=.45');
  } else {
  ...
  if (isMobile) {
    gsap.from(words,{yPercent:45,opacity:.12,stagger:.018,duration:.65,ease:'power3.out',scrollTrigger:{trigger:'.manifesto',start:'top 78%',toggleActions:'play none none reverse'}});
    gsap.from('.manifesto-note',{y:24,opacity:0,duration:.75,ease:'power3.out',scrollTrigger:{trigger:'.manifesto-note',start:'top 92%'}});
  } else {
  ...
  const storyTl=gsap.timeline({scrollTrigger:{trigger:'.story',start:'top top',end:'bottom bottom',scrub:isMobile ? .55 : 1,onUpdate:self=>setActive(Math.min(2,Math.floor(self.progress*3)))}});
  if (isMobile) {
    storyTl.to('.story-meter i',{scaleX:1,ease:'none',duration:1},0)
      .to([shots[0],steps[0]],{opacity:0,duration:.1,ease:'none'},.29)
      .fromTo(shots[1],{opacity:0},{opacity:1,duration:.12,ease:'none'},.31)
      .fromTo(steps[1],{opacity:0,y:16},{opacity:1,y:0,duration:.12,ease:'power2.out'},.33)
      .to([shots[1],steps[1]],{opacity:0,duration:.1,ease:'none'},.62)
      .fromTo(shots[2],{opacity:0},{opacity:1,duration:.12,ease:'none'},.64)
      .fromTo(steps[2],{opacity:0,y:16},{opacity:1,y:0,duration:.12,ease:'power2.out'},.66);
  } else {
    storyTl.to('.story-meter i',{scaleX:1,ease:'none',duration:1},0)
  ...
  let projectTween;
  let projectImageTweens=[];
  const setupProjects=()=>{
    projectImageTweens.forEach(t=>{t.scrollTrigger?.kill();t.kill();});
    projectImageTweens=[];
    if(projectTween){projectTween.scrollTrigger?.kill();projectTween.kill();projectTween=null;}
    gsap.set(track,{clearProps:'transform'});
    if(!desktopMq.matches){
      projectSection.style.height='auto';
      projectShell.style.position='relative';
      projectShell.style.top='auto';
      $$('.project img').forEach(img=>gsap.set(img,{clearProps:'transform'}));
      return;
    }
    ...
    projectImageTweens=$$('.project img').map(img=>gsap.to(img,{xPercent:-4,scale:1.13,ease:'none',scrollTrigger:{trigger:img.closest('.project'),containerAnimation:projectTween,start:'left right',end:'right left',scrub:true}}));
  };
  setupProjects();
  desktopMq.addEventListener?.('change',()=>{setupProjects();ScrollTrigger.refresh()});

  if (isMobile) {
    $$('.service').forEach(row=>gsap.from(row,{y:20,opacity:0,duration:.6,ease:'power3.out',scrollTrigger:{trigger:row,start:'top 94%'}}));
    gsap.to('.process-rail i',{scaleY:1,ease:'none',scrollTrigger:{trigger:'.process-list',start:'top 82%',end:'bottom 82%',scrub:true}});
    $$('.process-line').forEach(line=>gsap.from(line,{y:18,opacity:0,duration:.6,ease:'power3.out',scrollTrigger:{trigger:line,start:'top 92%'}}));
    gsap.from('.cta-inner h2',{y:20,opacity:0,duration:.75,ease:'power3.out',scrollTrigger:{trigger:'.cta',start:'top 72%'}});
  } else {
